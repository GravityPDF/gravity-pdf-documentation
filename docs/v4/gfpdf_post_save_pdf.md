...
It will be triggered when a Gravity Forms notification is sent AND the [PDF is configured to be attached to it](user-setup-pdf.md#notifications) – this includes during the initial form submission and when resending notifications. It will also be triggered during the form submission when the [*Always Save PDF*](user-setup-pdf.md#save-pdf) setting is enabled or when using [`GPDFAPI::create_pdf( $entry_id, $pdf_id )`](https://gravitypdf.com/documentation/v4/api_create_pdf/) method in our API. 
