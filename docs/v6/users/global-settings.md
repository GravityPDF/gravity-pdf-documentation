...
### Debug Mode
* When enabled, this turns on shortcode debugging and PDF statistics, turns off template caching, and [turns on the helper parameters](../developers/helper-parameters.md).
...
* _Template caching:_ for performance, Gravity PDF caches the list of installed custom templates, and the associated header data. The cache is flushed automatically when installing or deleting a template [via the PDF Template Manager](pdf-template-manager.md), or when Debug Mode is enabled.
