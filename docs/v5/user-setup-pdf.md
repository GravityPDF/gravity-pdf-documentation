...
2. Locate the Gravity Form you want to create a PDF on and hover your mouse (or similar device) over that particular row. A list of actions will be revealed; click the `Settings` action.
3. When the page loads, you'll see a new tab called `PDF` in the left sidebar. Click that link.
...
* The PDF [active state](https://gravity-pdf-documentation.onrender.com/v5/user-managing-pdfs#activate--deactivate-pdfs) and [*Conditional Logic*](#conditional-logic) determine if the PDF will be attached to the notification. If one or both of these tests fail, no PDF will be attached. This setting **does not** affect the actual notification conditional logic (which determines if the notification should be sent).
...
### Paper Size 
* Use this option to set the PDF paper size. This field defaults to the [Default Paper Size](https://gravity-pdf-documentation.onrender.com/v5/user-global-settings#default-paper-size) option in the global settings. Common paper sizes like US Letter and Legal are available. The ISO-standard *A*, *B* and *C* paper sizes, as well as *RA* and *SRA*, are also preloaded.
...
### Font 
* Select the font to be used in your PDF. This field defaults to the [Default Font](https://gravity-pdf-documentation.onrender.com/v5/user-global-settings#default-font-type) option in the global settings.
...
### Font Size 
* Control the font size used in the PDF. The field defaults to the [Default Font Size](https://gravity-pdf-documentation.onrender.com/v5/user-global-settings#default-font-size) option in the global settings.

### Font Colour 
* Control the font colour used in the PDF. The field defaults to the [Default Font Colour](https://gravity-pdf-documentation.onrender.com/v5/user-global-settings#default-font-colour) option in the global settings.

### Reverse Text (RTL) 
* Control the text direction used in the PDF. The field defaults to the [Reverse Text (RTL)](https://gravity-pdf-documentation.onrender.com/v5/user-global-settings#reverse-text-rtl) option in the global settings.
