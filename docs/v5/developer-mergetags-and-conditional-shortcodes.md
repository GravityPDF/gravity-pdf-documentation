...
In [Template Tutorial – Part 1](developer-first-custom-pdf.md#template-tutorial--part-1), we showed you the very minimum needed to create a PDF template. In part 2 we're going to utilise merge tags and conditional shortcodes in a template. Before you begin you will need to [download the Gravity Forms form](https://resources.gravitypdf.com/uploads/2015/11/hello-world-gravity-form.json) (right click and 'save link as') we'll be working with and [import it into your website](https://www.gravityhelp.com/documentation/article/importing-a-form-into-gravity-forms/). Once done, submit a new entry for the newly-imported *Hello World* form.
...
In our `hello-world.php` template, we're going to add conditional shortcodes that output a small fact about where the user lives. If a user selects `Earth`, only the Earth fact will be displayed. Below the line we just added we'll include the following:
